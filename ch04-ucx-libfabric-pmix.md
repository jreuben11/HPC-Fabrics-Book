...
This section installs UCX via libucx-dev and ucx-utils, which supply the transport abstraction library and the ucx_info and ucx_perftest tools used to enumerate available transports and measure point-to-point bandwidth and latency. The libfabric-dev and libfabric-bin packages provide LibFabric and its fi_info and fi_pingpong utilities for provider enumeration and baseline throughput measurement on whichever fabric is present. The libpmix-dev package installs the PMIx headers and runtime library needed to compile and run the wire-up examples, and OpenMPI is installed because it integrates both UCX and PMIx and serves as a convenient harness for exercising multi-rank launch and transport selection.
